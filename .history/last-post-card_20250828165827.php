...
<section class="px-2">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-xl lg:text-3xl md:text-2xl font-bold text-white <?= $lang === 'kh' ? 'khmer-font-kulen' : '' ?>"><?= $lang === 'en' ? 'Latest Posts' : 'ការបង្ហោះថ្មីៗ' ?></h1>
        <a href="last-post"
    ...
    <!-- Desktop grid -->
    <div class="hidden md:grid md:grid-cols-3 lg:grid-cols-4 gap-2 py-4">
                ...
                <div class="card bg-gray-800 rounded-lg shadow-md hover:shadow-lg overflow-hidden">
                    <?php if (!empty($p['image'])): ?>
                        <img src="./admin/posts/uploads/<?= htmlspecialchars($p['image']) ?>"
                            ...
                            class="w-full h-[90px] md:h-[130px] lg:h-[160px] rounded-none">
                    <?php endif; ?>
                ...
                <a href="detail.php?slug=<?= htmlspecialchars($p['slug']) ?>" class="flex-shrink-0">
                    <div
                        ...
                        <?php if (!empty($p['image'])): ?>
                            <img src="./admin/posts/uploads/<?= htmlspecialchars($p['image']) ?>"
                                alt="<?= htmlspecialchars($p['name']) ?>"
                                class="w-full h-[100px] sm:h-[120px] object-cover">
                        <?php endif; ?>
