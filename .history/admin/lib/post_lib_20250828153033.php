...
class Post
{
    ...
    // CREATE post
    public function createPost($name_en, $category_id, $subcategory_id = null, $description_en = null, $image = null, $name_kh = null, $description_kh = null, $added_by = null)
    {
        $slug = $this->generateSlug($name_en);
        $stmt = $this->db->prepare("
            INSERT INTO posts (name, name_kh, slug, category_id, subcategory_id, description, description_kh, image, added_by) 
            VALUES (:name_en, :name_kh, :slug, :category_id, :subcategory_id, :description_en, :description_kh, :image, :added_by)
        ");
        return $stmt->execute([
            'name_en' => $name_en,
            'name_kh' => $name_kh,
    ...
    public function getPostBySlug($slug, $lang = 'en')
    {
        ...
        $sql = "SELECT *,
            CASE WHEN :lang = 'kh' THEN name_kh ELSE name END AS name , link,
            CASE WHEN :lang = 'kh' THEN description_kh ELSE description END AS description, link
            FROM posts
            WHERE slug = :slug
            LIMIT 1";
