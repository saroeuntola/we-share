    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-w-7xl m-auto px-2">
        ...
        ?>
            <div class="bg-gray-800 rounded-lg shadow-md overflow-hidden">
                <a href="detail.php?slug=<?= htmlspecialchars($p['slug']) ?>&lang=<?= $lang ?>" class=" cursor-po">
                    <?php if (!empty($p['image'])): ?>
                        <img src=" ./admin/posts/uploads/<?= htmlspecialchars($p['image']) ?>"
                    alt="<?= htmlspecialchars($p['name']) ?>" class="w-full h-[90px] md:h-[130px] lg:h-[160px] object-cover rounded-none">
                <?php endif; ?>
