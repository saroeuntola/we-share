...
function navItem($page, $label, $lang, $currentPage, $currentSlug)
{
    ...
    $langClass = ($lang === 'kh') ? 'khmer-font-nav mt-1' : '';

    return "<a href=\"{$href}\" class=\"{$langClass} rounded-md text-base font-medium p-2 {$activeClass}\">{$label}</a>";
}
...
$menu = [
    ...
    'contact' => $lang === 'en' ? 'Contact' : 'សំណួរ',
    'privacy-policy' => $lang === 'en' ? 'Privacy Policy' : 'ចូលរួម',
    ...
    <div class="max-w-7xl mx-auto px-2">
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex items-center lg:hidden">
                ...
                <!-- Logo -->
                <a href="#" class="text-2xl font-bold text-white ml-3">
            ...
            <div class="hidden lg:flex justify-between items-center w-full">
                ...
                <div class="flex space-x-6 items-center">
                    ...
                    <div class="flex space-x-6 items-center">
                        ...
                        <?= navItem('contact', $menu['contact'], $lang, $currentPage, $currentSlug) ?>
                    ...
                    <div class="relative ">
                        ...
                        <button id="search-icon" class="text-white focus:outline-none flex justify-between items-center cursor-pointer">
                            <p​​ class="<?= $lang === 'kh' ? 'khmer-font-nav mt-1' : '' ?>"> <?= $lang === 'en' ? 'Search..' : 'ស្វែងរក...' ?></p​​>
                    ...
                    <div class="relative">
                        <button id="lang-btn-desktop" class="flex items-center text-white px-3 py-2 rounded-md hover:bg-green-600 focus:outline-none">
                            <img id="lang-flag-desktop" src="./assets/img/flags/<?= $lang === 'en' ? 'Flag_of_the_United_Kingdom' : 'khmer-flag' ?>.svg" class="w-6 h-4 ml-2" alt="Flag">
                            <span class="ml-2 font-medium <?= $lang === 'kh' ? 'khmer-font mt-1' : '' ?>"><?= $lang === 'en' ? 'English' : 'ភាសារខ្មែរ' ?></span>
                            ...
                            <?php foreach ($languageNames as $code => $name): ?>
                                <?php if ($code !== $lang): ?>
                                    <a href="<?= buildLangUrl($code, $currentPage, $currentSlug) ?>" class="flex items-center px-4 py-2 text-gray-700 hover:bg-gray-100 transition">
                                        <img src="./assets/img/flags/<?= $lang === 'kh' ? 'Flag_of_the_United_Kingdom' : 'khmer-flag' ?>.svg" class="w-6 h-4 mr-2" alt="<?= $name ?> Flag">
                                        <span class="text-right <?= $lang === 'en' ? 'khmer-font' : '' ?>"><?= $name ?></span>
                                    </a>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </div>
            ...
            <div class="flex items-center lg:hidden space-x-2">
                ...
                <div class="relative">
                    <button id="lang-btn-mobile" class="flex items-center text-white px-3 py-2 rounded-md hover:bg-green-600 focus:outline-none">
                        <img id="lang-flag-mobile" src="./assets/img/flags/<?= $lang === 'en' ? 'Flag_of_the_United_Kingdom' : 'khmer-flag' ?>.svg" class="w-6 h-4 ml-2" alt="Flag">
                        <span class="ml-2 font-medium <?= $lang === 'kh' ? 'khmer-font mt-1' : '' ?>"><?= $lang === 'en' ? 'English' : 'ភាសារខ្មែរ' ?></span>
                        <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
