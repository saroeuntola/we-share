...
<body class=" bg-gray-600">
    ...
    <main class="max-w-7xl mx-auto p-2 mt-[80px]">
        <div class="overflow-hidden mb-10">
            <h1 class="lg:text-3xl text-lg text-white font-bold mb-4">
            ...
            <div class="bg-gray-800 text-white p-3">
                ...
                <!-- Text content -->
                <p class="mb-3">
                    <span class="font-semibold"><?= $lang === 'en' ? 'Category:' : 'ប្រភេទៈ' ?></span> <?= $category ? htmlspecialchars($category['name']) : '-' ?>
                    <span class="font-semibold">,</span> <?= $subcategory ? htmlspecialchars($subcategory['name']) : '-' ?><br>
                    <span class="font-semibold"><?= $lang === 'en' ? 'Post by:' : 'បង្ហោះដោយៈ' ?></span> <?= htmlspecialchars($post['added_by']) ?><br>
                    <span class="font-semibold"><?= $lang === 'en' ? 'Post Date:' : 'កាលបរិច្ឆេទៈ' ?></span> <?= htmlspecialchars($post['created_at']) ?>
                </p>

                <div class="text-gray-200">
                    <?= $post['description'] ?>
                </div>
                <p class=" text-red-500 rounded-lg text-sm md:text-base mt-4">
                    <strong>Note​: <?= $lang === 'en' ? 'Post by:' : 'បង្ហោះដោយៈ' ?></strong> Documents or software hosted on <span class="font-semibold">weshare.com</span> are not owned by this site. If you are the copyright owner and wish to request removal, please <a href="/contact" class="underline text-blue-600 hover:text-blue-800">contact us</a> and we will remove it promptly.
                </p>
