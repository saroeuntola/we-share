...
<head>
    ...
    <title><?= $lang === 'en' ? 'Latest Posts' : 'ការបង្ហោះថ្មីៗ' ?></title>
...
<body class="bg-gray-600">
    ...
    <main class="max-w-7xl m-auto p-4 mt-[80px]">
        <div class="">
            <a href="index?lang=<?= $lang ?>"
                class="inline-flex justify-end items-center text-white hover:text-green-400 transition mb-6 <?= $lang === 'kh' ? 'khmer-font mt-1' : '' ?>">
                <!-- Arrow Icon -->
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2"
                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
                <p><?= $lang === 'en' ? 'Back Home' : 'ត្រឡប់ទៅទំព័រដើម' ?></p>

            </a>
            <div class="flex items-center justify-between mb-2">
                <h1 class="lg:text-3xl text-2xl font-bold text-white <?= $lang === 'kh' ? 'khmer-font-kulen' : '' ?>"><?= $lang === 'en' ? 'Latest Posts' : 'ការបង្ហោះថ្មីៗ' ?></h1>

                <!-- CATEGORY FILTER -->
                <div class="relative inline-block">
                    <button id="dropdownButton" type="button"
                        class="text-white hover:text-green-600 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center <?= $lang === 'kh' ? 'khmer-font' : '' ?>"
                        onclick="toggleDropdown('categoryDropdown')">
                        <?= $categorySlug
                            ? htmlspecialchars($categoryModel->getCategory($categorySlug)['name'])
                            : ($lang === 'en' ? 'All Categories' : 'ប្រភេទៈ​ ទាំងអស់') ?>
                        ...
                        <ul class="py-2 text-sm text-gray-700">
                            <li>
                                <a href="<?= $_SERVER['PHP_SELF'] ?>" class="block px-4 py-2 hover:bg-gray-100">
                                    <?= $lang === 'en' ? 'All Categories' : '​ទាំងអស់' ?>
                                </a>
                            </li>
                            <?php foreach ($categoryModel->getCategories() as $catOption): ?>
                                <li>
                                    <a href="?category=<?= htmlspecialchars($catOption['slug']) ?>&lang=<?= $lang ?>"
    ...
    <?php if (!empty($subcategories)): ?>
        <div class="mb-4 flex flex-wrap gap-2 max-w-7xl m-auto px-4">
    ...
    <!-- POSTS GRID -->
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-w-7xl m-auto px-4">
                ...
                <a href="detail.php?slug=<?= htmlspecialchars($p['slug']) ?>&lang=<?= $lang ?>" class=" cursor-pointer">
                    <?php if (!empty($p['image'])): ?>
                        <img src=" ./admin/posts/uploads/<?= htmlspecialchars($p['image']) ?>"
                            alt="<?= htmlspecialchars($p['name']) ?>" class="w-full h-[120px] md:h-[130px] lg:h-[180px] rounded-none">
                    <?php endif; ?>
