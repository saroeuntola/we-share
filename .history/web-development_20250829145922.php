...
<head>
    ...
    <title><?= $lang === 'en' ? 'Web Development' : 'ឯកសារ អភិវឌ្ឍន៍គេហទំព័រ' ?></title>
...
<body class="bg-gray-600">
    ...
    <main class="max-w-7xl m-auto p-4 mt-[80px]">
        <a href="index?lang=<?= $lang ?>"
            class="inline-flex justify-end items-center text-white hover:text-green-400 transition mb-4 <?= $lang === 'kh' ? 'khmer-font mt-1' : '' ?>">
            <!-- Arrow Icon -->
        ...
        <div class=" justify-between items-center gap-4 mb-5">
            <h2 class="lg:text-2xl text-[20px] font-bold text-white <?= $lang === 'kh' ? 'khmer-font-kulen' : '' ?>"><?= $lang === 'en' ? 'Web Development' : 'ឯកសារ អភិវឌ្ឍន៍គេហទំព័រ' ?></h2>

            <div id="subcategoryDropdown" class="flex flex-wrap gap-2 mt-4">
                <a href="?sub="
                    class="px-4 py-2 rounded-lg text-sm font-medium <?= $lang === 'kh' ? 'khmer-font' : '' ?>
              <?php echo empty($filterSubSlug)
                ...
                <?php foreach ($subcategories as $sub): ?>
                    <a href="?sub=<?php echo htmlspecialchars($sub['slug']); ?>&lang=<?= $lang ?>"
