...
<section class="px-2">
    <div class="flex justify-between items-center">
        <h1 class="<div class=" hidden md:grid md:grid-cols-3 lg:grid-cols-4 gap-2 py-5 mb-4">
            <a href="detail?slug=download-php-programming-khmer-pdf&amp;lang=en">
                <div class="card bg-gray-800 rounded-lg shadow-md hover:shadow-lg overflow-hidden">
                    <img src="./admin/posts/uploads/1756373865_Screenshot_2025-08-28_163455.png" alt="Download PHP Programming Khmer pdf" class="w-full h-[90px] md:h-[130px] lg:h-[180px] rounded-none">
                    <div class="card-body px-4 py-2">
                        <h2 class="font-semibold text-lg mb-1">Download PHP Programming Khmer pdf</h2>

                    </div>
                </div>
            </a>

    </div> font-bold text-white <?= $lang === 'kh' ? 'khmer-font-kulen' : '' ?>"><?= $lang === 'en' ? 'Programming' : 'ឯកសារ រសរសេរកម្មវិធី' ?></h1>
    <a href="programming?lang=<?= $lang ?>"
                ...
                <div class="card bg-gray-800 rounded-lg shadow-md hover:shadow-lg overflow-hidden">
                    <?php if (!empty($p['image'])): ?>
                        <img src="./admin/posts/uploads/<?= htmlspecialchars($p['image']) ?>"
                            ...
                            class="w-full h-[90px] md:h-[130px] lg:h-[180px] rounded-none">
                    <?php endif; ?>
                    <div class="card-body px-4 py-2">
    ...
    <div class="overflow-x-auto md:hidden py-2">
        <div class="flex space-x-2">
            <?php foreach ($posts as $p): ?>
                <a href="detail.php?slug=<?= htmlspecialchars($p['slug']) ?>&lang=<?= $lang ?>" class="flex-shrink-0">
                    <div
